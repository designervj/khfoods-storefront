      ...
      <div className="container-custom">
        {header && (
          <div className="text-center mb-12">
            <EditableText value={getLocalizedString(header.props?.heading, locale)} onSave={createSaveHandler(header.id, 'props.heading')} isEditable={isEditable} tag="h1" className="text-4xl md:text-5xl font-bold mb-4" placeholder="FAQ heading..." />
            <EditableText value={getLocalizedString(header.props?.subheading, locale)} onSave={createSaveHandler(header.id, 'props.subheading')} isEditable={isEditable} tag="p" className="text-lg text-[var(--text-secondary)] max-w-2xl mx-auto" placeholder="FAQ subheading..." multiline rows={2} />
          </div>
        )}
            ...
            {accordion.content.map((item, idx) => (
              <div key={item.id} className="card-theme overflow-hidden">
                <button onClick={() => setOpenId(openId === item.id ? null : item.id)} className="w-full flex items-center justify-between p-6 text-left hover:bg-[var(--surface)] transition-colors">
                  <EditableText value={getLocalizedString(item.props?.question, locale)} onSave={createSaveHandler(accordion.id, `content.${idx}.props.question`)} isEditable={isEditable} tag="span" className="font-semibold text-lg" placeholder="Question..." />
